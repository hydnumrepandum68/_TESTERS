...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/22 12:10:00 by eguthman          #+#    #+#             */
/*   Updated: 2024/11/22 16:17:30 by eguthman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	char	*str2 = "";
	char	*str3 = NULL;
	void	*ptr1 = (void *)0; // pointeur generique a valeur nulle qui ne pointe vers aucune adresse valide
	void	*ptr2 = (void *)&a; // pointeur générique qui stocke l'adresse de la variable a
	int		*dynamic_ptr = (int *)malloc(sizeof(int)); // alloue dynamiquement de la mémoire suffisante pour stocker un entier et initialise le ptr avec l'adresse de cette mémoire
	void	*max_ptr  = (void *)-1; // adresse mémoire spéciale, où tous les bits sont à 1. Pointeur spécial en dehors de la plage normale d’adresses mémoire valides
	int		return_value_ft;
	...
	ft_printf("Unsigned int max + 1 (overflow en recommancant a 0 : %u\n", UINT_MAX + 1);
	printf("Unsigned int max + 1 (overflow en recommancant a 0) : %u\n", UINT_MAX + 1);
	ft_printf("Unsigned int max * -1 (-4294967295) %% 2^32 = 1) : %u\n", UINT_MAX * -1);
	printf("Unsigned int max * -1 (-4294967295) %% 2^32 = 1) : %u\n", UINT_MAX * -1);
	...
	return_value_ft = ft_printf("0\n");
	printf("ft_printf renvoie: %d\n", return_value_ft);
	return_value_std = printf("0\n");
	printf("printf renvoie: %d\n", return_value_std);
	return_value_ft = ft_printf("%u\n", -42);
	printf("ft_printf renvoie: %u\n", return_value_ft);
	return_value_std = printf("%u\n", -42);
	printf("printf renvoie: %u\n", return_value_std);
	
	printf("\n--- Tests avec des conversions ---\n");
	return_value_ft = ft_printf("Nombre 42 : %d, Hex 42 : %x\n", 42, 42);
	printf("ft_printf renvoie: %d\n", return_value_ft);
	return_value_std = printf("Nombre 42 : %d, Hex 42 : %x\n", 42, 42);
	printf("printf renvoie: %d\n", return_value_std);
	return_value_ft = ft_printf("%x\n", 0);
	printf("ft_printf renvoie: %x\n", return_value_ft);
	return_value_std = printf("%x\n", 0);
	printf("printf renvoie: %x\n", return_value_std);
	return_value_ft = ft_printf("%X\n", 0);
	printf("ft_printf renvoie: %X\n", return_value_ft);
	return_value_std = printf("%X\n", 0);
	printf("printf renvoie: %X\n", return_value_std);
